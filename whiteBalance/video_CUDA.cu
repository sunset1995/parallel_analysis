...
void whiteBalance_CUDA(Mat imgs[], const int &sz) {
	...
	err = cudaMalloc(&device_r, size); if (err != cudaSuccess) fprintf(stderr, "Failed while allocating memory! %s\n", cudaGetErrorString(err));
	err = cudaMalloc(&device_g, size); if (err != cudaSuccess) fprintf(stderr, "Failed while allocating memory! %s\n", cudaGetErrorString(err));
	err = cudaMalloc(&device_b, size); if (err != cudaSuccess) fprintf(stderr, "Failed while allocating memory! %s\n", cudaGetErrorString(err));
	
	err = cudaMemcpy(device_r, host_r, size, cudaMemcpyHostToDevice); if (err != cudaSuccess) fprintf(stderr, "Failed while cudaMemcpy! %s\n", cudaGetErrorString(err));
	err = cudaMemcpy(device_g, host_g, size, cudaMemcpyHostToDevice); if (err != cudaSuccess) fprintf(stderr, "Failed while cudaMemcpy! %s\n", cudaGetErrorString(err));
	err = cudaMemcpy(device_b, host_b, size, cudaMemcpyHostToDevice); if (err != cudaSuccess) fprintf(stderr, "Failed while cudaMemcpy! %s\n", cudaGetErrorString(err));
	...
	if (err != cudaSuccess) fprintf(stderr, "Failed to launch kernel (error code %s)!\n", cudaGetErrorString(err));
	...
	for (k = 0; k < sz; k++)
		for (i = 0; i < rows; i++)
			for (j = 0; j < cols; j++) {
				imgs[k].at<Vec3b>(i, j)[2] = host_r[iter];
				imgs[k].at<Vec3b>(i, j)[0] = host_b[iter];
				iter++;
			}
